...
Describe "Json Tests" -Tags "Feature" {
    ...
    Context "ConvertTo-Json Bug Fixes" {
        ...
        It "ConvertTo-JSON should not have hard coded english error message" {
            ...
            $hash = @{ $process = "def" }
            $expectedFullyQualifiedErrorId = "NonStringKeyInDictionary,mymodule.ConvertToJsonCommand"
...
Describe "Json Bug fixes"  -Tags "Feature" {
    ...
    $testCases = @(
        @{
            ...
            NumberOfElements = 10
            MaxDepth = 101
            FullyQualifiedErrorId = "ParameterArgumentValidationError,mymodule.ConvertToJsonCommand"
            ShouldThrow = $true
        }
        @{
